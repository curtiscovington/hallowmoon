...
- **Timers:** Present both countdown text and visual progress when an action can expire. Freeze progress bars when time is paused.
  - *Exception:* SlotMap territory markers rely solely on the circular progress ring to keep location names unobstructed; announce remaining duration via accessible labels instead of inline text.
...
- **Intro overlay:** When onboarding a new caretaker, open a dedicated overlay before any other modals. Pause the simulation while it is visible, step through no more than four slides, and close on an explicit confirmation so players can orient themselves before time resumes.
- **Persona selection overlay:** Greet first-time players with the invocation overlay before the intro slides. It should cloak the board with a full-screen blur, present three persona cards in a responsive grid (minimum 220 px wide each), and accent each card with a gradient glow that intensifies on hover/focus. The copy “To exist here…” must precede the cards verbatim to ground the tone, and selecting any persona should immediately lock in the choice, drop the card into the hand, and keep time paused until the player dismisses the subsequent intro.
- **Territory maps:** Use the SlotMap board for any spatial view of slots. Keep the illustrated base as decorative and rely on outlined square markers with short labels so the artwork remains visible. Ensure markers support keyboard focus, drag targets, and finger taps without requiring precision; pair them with a dedicated detail panel so the map stays uncluttered. Provide zoom controls that respond to taps, wheels, and pinch-zoom modifiers while keeping the buttons comfortably within thumb reach on mobile. Each marker should surface the current occupant with a miniature card tile and expose the primary action as a floating pill when the slot is ready—this keeps the map actionable at a glance. Clicking a marker opens the full slot card in a modal overlay so flavor text and controls remain accessible without leaving the map.
- **Story beat toasts:** Surface major narrative beats as floating toasts anchored just above the hand controls. They should auto-dismiss after about four seconds, respect safe-area insets, and avoid blocking pointer input by default. Mirror the dark glass treatment used elsewhere and ensure the enter animation shortens to none under `prefers-reduced-motion`.
